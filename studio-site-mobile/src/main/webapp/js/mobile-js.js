...
function center(){
			...
			$("p.dDetailsTitleP").css({"width":tWidth+"px","margin-left":(-(tWidth/2))+"px","left":50+"%"});
			
			var tWidth1 = $(".dDetailsTitleP1").outerWidth(true);
			
			$(".dDetailsTitleP1").css({"width":tWidth1+"px","margin-left":(-(tWidth1/2))+"px","left":50+"%"});
...
/*	
	...
	//首页banner轮播图
	var bannerNum = 0;
	var aLi = $("#bannerList").children("li");
	var timer = null;
	
	$("#bannerList").on("swipeleft","li",function(event){
		
		bannerNum++;
		
		if(bannerNum > aLi.length-1){
	...
	$("#bannerList").on("swiperight","li",function(event){
		
		bannerNum--;
		
		if(bannerNum < 0){
			
			bannerNum = aLi.length-1;
	...
	function imgShow(){
		...
		aLi.eq(bannerNum).stop(true,true).fadeIn("fast").siblings().hide();
		
		$("#bannerNumList").children("li").eq(bannerNum).addClass("active").siblings().removeClass("active");
		
	}
	
	for(var i=0; i< aLi.length; i++){
		
		$("<li></li>").appendTo($("#bannerNumList")); 
		
	}
	$("#bannerNumList").css({"width":(aLi.length*20)+"px","margin-left":(-(aLi.length*20)/2)+"px"});
	$("#bannerNumList li").first().addClass("active");
	//自动播放
...
//摄影师档期选择
	...
	(function(){
		...
		sDateLi.on("tap",function(){
			...
			if(!$(this).hasClass("sDisable")){
				...
				//设置选中时间的键名  赵永丹
				jQuery("input[name=surplus]").val($(this).children().children(".sDateLeft").attr('ref'));
	...
	(function(){
		...
		dSecondImgList.css({"width":docW + "px","height":docH + "px","overflow":"hidden"});
		dSecondImg.css({"width":docW + "px","height":(docW*2)/3 + "px"});
	...
	/*sSysList.on("mouseup","li",function(){
